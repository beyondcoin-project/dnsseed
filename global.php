...
/*
// Functions used by beyondcoin-scan.php and beyondcoin-scan-net.php
function connect_to_db() {
	global $db, $CONFIG;
	try {
		$db = new mysqli($CONFIG['MYSQL_HOST'], $CONFIG['MYSQL_USER'], $CONFIG['MYSQL_PASS'], $CONFIG['MYSQL_BEYONDCOIN_DB']);
		if ($db->connect_errno)
...
// Functions used only by beyondcoin-scan.php
...
function add_node_to_dns($ip, $port, $version) {
	...
	if (!empty($ip) && ip2long($ip) != 0 && !empty($port) && is_numeric($port) && $port != 0 && is_numeric($version) && $version > 0) {
		$db->query("INSERT INTO `".$CONFIG['MYSQL_BEYONDCOIN_TABLE']."` "
			."(`ipv4`, `port`, `accepts_incoming`, `last_check`, `version`, `first_up`) VALUES "
			."('" . ip2long($ip) . "', '" . $port . "', b'1', NOW(), '" . $version . "', NOW());");
		$db->query("UPDATE `".$CONFIG['MYSQL_BEYONDCOIN_TABLE']."` SET "
	...
	if (!empty($ip) && ip2long($ip) != 0 && $version >= $CONFIG['MIN_VERSION'] && $port == 10333)
...
// Functions used only by beyondcoin-scan-net.php
function scan_node($ip, $port) {
	exec("nohup ./beyondcoin-scan.php ".long2ip($ip).":".$port." > /dev/null 2>/dev/null &");
}

function query_unchecked() {
	global $db, $CONFIG;
	return $result = $db->query("SELECT `ipv4`, `port` FROM `".$CONFIG['MYSQL_BEYONDCOIN_TABLE']."` WHERE `last_check` IS NULL;");
}

function query_unaccepting() {
	global $db, $CONFIG;
	return $db->query("SELECT `ipv4`, `port` FROM `".$CONFIG['MYSQL_BEYONDCOIN_TABLE']."` WHERE `last_check` < NOW() - INTERVAL " . $CONFIG['UNACCEP_CHECK_RATE'] . " SECOND AND `accepts_incoming` = b'0' ORDER BY `last_check` ASC;");
}

function query_accepting() {
	global $db, $CONFIG;
	return $db->query("SELECT `ipv4`, `port` FROM `".$CONFIG['MYSQL_BEYONDCOIN_TABLE']."` WHERE `last_check` < NOW() - INTERVAL " . $CONFIG['ACCEP_CHECK_RATE'] . " SECOND AND `accepts_incoming` = b'1' ORDER BY `last_check` ASC;");
}
...
function prune_nodes() {
	global $db, $CONFIG;
	$db->query("DELETE FROM `".$CONFIG['MYSQL_BEYONDCOIN_TABLE']."` WHERE `last_seen` < NOW() - INTERVAL " . $CONFIG['PURGE_AGE'] . " SECOND AND `accepts_incoming` = b'0';");
}*/
...
function connect_to_db() {
	...
	$db->exec("CREATE TABLE IF NOT EXISTS nodes (
			ipv4 INT NOT NULL,
			port INT NOT NULL DEFAULT 10333,
...
function get_list_of_nodes_for_dns() {
	global $db, $CONFIG;
	return $db->query("SELECT ipv4 FROM nodes WHERE accepts_incoming = 1 AND port = 10333 AND version >= ".$CONFIG['MIN_VERSION']." AND subversion IN ".$CONFIG['SUBVERSIONS']." ORDER BY last_check DESC LIMIT 20;");
}

// Functions used only by count-nodes.php
function query_version_count() {
	global $db, $CONFIG;
	return $db->query("SELECT COUNT(*), subversion AS version FROM nodes WHERE accepts_incoming = 1 AND port = 10333 GROUP BY subversion ORDER BY subversion;");
}

function query_dns_total() {
	global $db, $CONFIG;
	return $db->query("SELECT COUNT(*) FROM nodes WHERE accepts_incoming = 1 AND port = 10333 AND version >= ".$CONFIG['MIN_VERSION']." AND subversion IN ".$CONFIG['SUBVERSIONS'].";");
}
